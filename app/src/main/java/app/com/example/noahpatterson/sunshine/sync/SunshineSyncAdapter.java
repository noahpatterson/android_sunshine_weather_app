package app.com.example.noahpatterson.sunshine.sync;
...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    @IntDef({LOCATION_STATUS_OK,LOCATION_STATUS_SERVER_DOWN,LOCATION_STATUS_SERVER_INVALID,LOCATION_STATUS_UNKNOWN,LOCATION_UNKNOWN})
    ...
    private static final String SYNC_LOCATION_STATUS = "sync_location_status";
    ...
    private void setLocationStatus(@LocationStatus int locationStatus) {
        SharedPreferences locationSetting = PreferenceManager.getDefaultSharedPreferences(getContext());
...
//        String location = intent.getStringExtra("location_to_fetch");
...
//                    "http://www.google.com/?";
            ...
            getWeatherDataFromJson(forecastJsonStr, location, LOG_TAG, getContext());
        } catch (IOException e) {
    ...
    private void notifyWeather() {
        ...
        if (prefs.getBoolean("notification", true)) {
            ...
            if (System.currentTimeMillis() - lastSync >= 1000 * 60) { //DAY_IN_MILLIS) {
                ...
                if (cursor.moveToFirst()) {
                    ...
                    NotificationCompat.Builder notifiyBuilder = new NotificationCompat.Builder(context)
                            .setSmallIcon(iconId)
                            .setContentTitle(title)
                            .setContentText(contentText);

                    // create explicit intent to the app on notification click
                    ...
                    notifiyBuilder.setContentIntent(resultPendingIntent);
                    NotificationManager mNotificationManager =
                            (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);

                    // mId allows you to update the notification later on.
                    mNotificationManager.notify(WEATHER_NOTIFICATION_ID, notifiyBuilder.build());
    ...
    private long addLocation(String locationSetting, String cityName, double lat, double lon, Context mContext) {
        ...
        Cursor weatherInDB = mContext.getContentResolver().query(
        ...
        if (weatherInDB.moveToFirst()) {
            try {
                int indexOfIDColumn = weatherInDB.getColumnIndex(WeatherContract.LocationEntry._ID);
                return weatherInDB.getLong(indexOfIDColumn);
            }
            finally {
                weatherInDB.close();
            }
            ...
            Uri insertedLocationURI = mContext.getContentResolver().insert(WeatherContract.LocationEntry.CONTENT_URI, locationValues);
...
//        try {
            ...
            if (forecastJson.has(OWM_MESSAGE_CODE)) {
                ...
                long locationId = addLocation(locationSetting, cityName, cityLatitude, cityLongitude, mContext);
